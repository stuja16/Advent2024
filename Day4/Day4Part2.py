...
# https://www.reddit.com/r/adventofcode/comments/1h689qf/comment/m0bsh3p/
...
def findMASes(grid):
    ...
    for y, x in grid:
        if grid[y,x] == "A":
            lr = grid.get((y-1,x-1),"")+grid.get((y+1,x+1),"")  # top-left to bot-right diag
            rl = grid.get((y+1,x-1),"")+grid.get((y-1,x+1),"")  # top-right to bot-left diag
